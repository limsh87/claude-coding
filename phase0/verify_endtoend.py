...
시장 스냅샷 provider 와 DART 를 바깥에서 가짜로 갈아끼워 오케스트레이션·판정표·산출물
생성까지 돌린다. 배포 파일 안에는 합성 경로가 없다(P0_LIVE_ONLY) — 주입은 전적으로
이 테스트 쪽에서 한다.

가장 중요한 시나리오는 [17]: pykrx 가 완전히 죽은 상태(실제로 사용자 환경에서 난 크래시)
에서도 다른 provider 만으로 4개 스냅샷·3개 전이가 끝까지 완주되는가다.
"""
...
# ══════════════════════════════════════════════════════════════════════════════════════════
#  공용 픽스처 — 2,881종목 합성 시장 (실측 규모와 동일하게 맞춤)
# ══════════════════════════════════════════════════════════════════════════════════════════
N_STK, N_KSQ, N_KNX = 950, 1822, 109          # 2026-03-31 실측 분포와 동일
CODES = ([f"{i:06d}" for i in range(1, N_STK + N_KSQ + 1)] +
         [f"{i:06d}" for i in range(900001, 900001 + N_KNX)])
MKTS = ["STK"] * N_STK + ["KSQ"] * N_KSQ + ["KNX"] * N_KNX
NCO = N_STK + N_KSQ                            # KONEX 제외 상장사 = 2,772 (실측과 동일)
...
def stub_provider(date_iso: str):
    """날짜당 1회 호출되는 시장 스냅샷 provider 의 대역."""
    _provider_calls.append(date_iso)
...
def reset_market_state():
    ...
    if M.DIR_CACHE_MARKET:
        for f in Path(M.DIR_CACHE_MARKET).glob("krx_market_*.csv"):
            f.unlink()
...
def _dirs():
    ...
    for d in (M.DIR_CACHE_RAW, M.DIR_CACHE_META, M.DIR_STATE, M.DIR_REPORTS, M.DIR_CACHE_MARKET):
...
def _boot_pykrx_dead():
    """[17] 전용: 사용자 환경에서 실제로 난 크래시 이후 상태를 main() 안에서 그대로 재현한다.
    이게 없으면 main() 이 부르는 _bootstrap 이 매번 PYKRX_AVAILABLE 을 되돌려 무의미해진다."""
    _dirs()
    M.PYKRX_AVAILABLE = False
    M.stock = None
    M.PYKRX_DIAGNOSTIC = ("JSONDecodeError: Expecting value: line 13 column 1 (char 25) "
                          "(시뮬레이션 — 사용자 환경에서 실제로 난 크래시)")
...
def seed_officer_cache():
    """임원 캐시를 원본 응답 형태로 심는다. 스냅샷마다 겸직 구조를 바꿔 born/died 를 만든다."""
...
M.MARKET_PROVIDERS = [("stub", stub_provider)]
_boot()
reset_market_state()
seed_corpcode()
seed_officer_cache()
...
for sid in ("S1", "S2", "S3", "S4"):
    ...
    check(f"{sid} 노드 = KONEX 제외 {NCO}종목", s[sid]["nodes"], NCO)
    check(f"{sid} KONEX {N_KNX}종목이 집계되고 제외됨", s[sid]["n_konex"], N_KNX)
    check(f"{sid} 엣지 > 0", s[sid]["edges"] > 0, True)
    check(f"{sid} 시총소스 기록", s[sid]["mcap_source"], "stub")
...
# ── 시장 스냅샷 캐시가 실제로 네트워크를 줄이는가 (이번 개정의 핵심 성능 주장) ──────────
n_calls_run1 = len(_provider_calls)
check("1회차 provider 호출은 유한하다(스냅샷4 + 휴장역행1 + 건강검진1 수준)",
      n_calls_run1 <= 12, True)
_provider_calls.clear()
M._SNAP_MEM.clear()                     # 메모만 비우고 캐시 파일은 남긴다
V2 = M.main()
check("2회차 결과 동일 (결정적)",
      [x["edge_born"] for x in V2["transitions"]], [x["edge_born"] for x in t])
check("2회차도 신규 DART 호출 0건", V2["cache"]["actual_new_calls"], 0)
# 2회차에 남는 호출은 (a) 건강검진 1회 (b) 휴장일 재확인이다.
# '데이터 없음'은 의도적으로 디스크에 캐시하지 않는다 — 오늘의 404 가 내일은 실데이터일 수
# 있고(소스 백필/일시 장애), 그걸 영구 캐시하면 없는 데이터를 영원히 없다고 믿게 된다.
# 확정된 거래일 스냅샷만 캐시하므로 재실행 비용은 종목수와 무관하게 상수로 유지된다.
check("2회차 시장 provider 호출이 상수로 줄어든다(스냅샷 4개는 전부 캐시 히트)",
      len(_provider_calls) <= 3, True)
_snap_dates = {u["date"] for u in V["snapshots"] if u["date"]}
check("2회차에 확정된 스냅샷 거래일은 단 한 번도 재조회하지 않는다(전부 캐시)",
      sorted(set(_provider_calls) & _snap_dates), [])
mkt_cache_files = list(Path(M.DIR_CACHE_MARKET).glob("krx_market_*.csv"))
check("시장 스냅샷이 로컬 캐시 파일로 남는다", len(mkt_cache_files) >= 4, True)
shared_mkt = list((Path(M.GDRIVE_ROOT) / M.GDRIVE_SHARED_NS / "table").glob("krx_market_*.csv"))
check("시장 스냅샷이 드라이브 공용 인덱스에도 게시된다(다른 전략 재사용)",
      len(shared_mkt) >= 4, True)
n_synced = sum(1 for _ in (Path(M.GDRIVE_ROOT) / M.GDRIVE_SHARED_NS / "blob" / "dart"
                           / "exctv").rglob("*.json"))
check("임원 blob 도 드라이브에 증분 동기화됨", n_synced > 0, True)

# ══════════════════════════════════════════════════════════════════════════════════════════
print("\n[17] pykrx 완전 사용불가 → 다른 provider 만으로 완주 (사용자 크래시 재현)")
...
class _FakeRequests:
    """KRX Open API 만 응답한다. 이 시나리오에서 pykrx 는 죽어 있으므로,
    실제 snap_krx_openapi() 구현이 그대로 실행되어 검증된다."""
    @staticmethod
    def get(url, timeout=None, **kw):
        ...
        try:
            iso = f"{d[:4]}-{d[4:6]}-{d[6:]}"
            trading = _is_trading(iso)
        except Exception:
            trading = False
        if not trading:
            return _FakeResp({"OutBlock_1": []})
        rows = [{"ISU_SRT_CD": c, "MKTCAP": f"{int(CAPS[c]):,}",
                 "MKT_NM": {"STK": "KOSPI", "KSQ": "KOSDAQ", "KNX": "KONEX"}[m]}
                for c, m in zip(CODES, MKTS)]
        return _FakeResp({"OutBlock_1": rows})


_real_requests = M.requests
M.requests = _FakeRequests
M.MARKET_PROVIDERS = _real_providers            # 진짜 provider 체인을 태운다
...
check("KRX Open API 가 실제로 폴백을 담당했다",
      V3["data_sources"]["krx_openapi_available"], True)
s3 = {x["id"]: x for x in V3["snapshots"]}
for sid in ("S1", "S2", "S3", "S4"):
    check(f"{sid} 상태 OK (pykrx 없이도)", s3[sid]["status"], "OK")
    check(f"{sid} 시총소스 = krx_openapi", s3[sid]["mcap_source"], "krx_openapi")
    check(f"{sid} 노드 = KONEX 제외 {NCO}종목 (콤마 박힌 MKTCAP 파싱 포함)",
          s3[sid]["nodes"], NCO)
check("S3 휴장일 스냅이 폴백 경로에서도 동작", s3["S3"]["date"], "2025-12-30")
t3 = V3["transitions"]
...
# ══════════════════════════════════════════════════════════════════════════════════════════
print("\n[18] 모든 provider 사망 → 전 스냅샷 UNVERIFIED, HardStop")
...
try:
    M.main()
except M.HardStop as e:
    raised = str(e)
check("모든 소스가 죽으면 HardStop (조용히 빈 결과를 내지 않는다)", raised is not None, True)
check("죽은 소스는 몇 번만 호출되고 이후 생략됨 "
      "(이전 판은 11일×4시그니처×4스냅샷=176회를 헛돌았다)",
      len(_provider_calls) <= M.PROVIDER_DEAD_AFTER + 1, True)
budget_files = list((Path(M.PROJECT_ROOT).expanduser() / "cache" / "state").glob("call_budget_*.json"))
check("DART 임원 호출은 0건 (유니버스 실패 시 수집 진입 금지)", budget_files, [])

# ══════════════════════════════════════════════════════════════════════════════════════════
print("\n[19] S3 만 전면 휴장 → 그 스냅샷만 UNVERIFIED, 나머지는 정상")
...
for back in range(M.TRADING_DAY_LOOKBACK + 1):
    HOLIDAYS.add((_dt.date(2025, 12, 31) - _dt.timedelta(days=back)).isoformat())
...
# ══════════════════════════════════════════════════════════════════════════════════════════
print("\n[20] 신규 호출 5,000건 초과 → 수집 전 정지 (DART 호출 0건인 채로)")
M.PROJECT_ROOT = str(TMP / "proj20")
M.GDRIVE_ROOT = ""
...
check("정확한 예정 호출수 보고 (KONEX 제외 4스냅샷)", V5["planned_calls"], 4 * NCO)
