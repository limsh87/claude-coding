...
def run_contract_tests(strict: bool = True) -> bool:
    ...
    def a3():
        ...
        if r1[1] == r3[1]:
            return False, "★시총 랭크가 입력 시총에 반응하지 않습니다 — 시총을 뒤집었는데 1순위 종목이 같습니다. PIT 시총이 아니라 다른 것으로 랭크하고 있을 가능성이 큽니다."
    ...
    def a4():
        ...
        for r in Pr.itertuples(index=False):
            if str(r.rcept_no).endswith("FY") != str(r.prev_rcept_no).endswith("FY"):
                return False, "★문서 유형이 섞인 페어가 생성되었습니다. 사업보고서와 분기보고서는 분량·구성이 구조적으로 달라 가짜 변화가 대량 발생합니다(§6.1.1)."
    ...
    def a5():
        ...
        if j1 < 0.98:
            return False, (
                f"★숫자·날짜·사명만 바꾼 문서의 자카드가 {j1:.3f} 입니다(기대 ≥0.98). 정규화가 가짜 변화를 제거하지 못했다는 뜻이며, 이 상태에서는 D1 의 모든 결과가 무의미합니다 — 전 종목이 '변경 기업'이 됩니다.")
        if j2 >= j1 - 0.02:
            return False, (
                f"★실제 서술을 바꾼 문서의 자카드가 {j2:.3f} 로 가짜변화본({j1:.3f})과 구분되지 않습니다. 정규화가 과도해서 진짜 변화까지 지워버렸을 가능성이 큽니다.")
    ...
    def a7():
        ...
        if abs(float(comp[0]) - 1.0) > 1e-9:
            return False, (
                f"★결측 섹션 가중치가 비례 재배분되지 않았습니다 (전 섹션 값이 1.0 인데 합성이 {comp[0]:.4f}). 섹션이 적게 파싱된 종목이 구조적으로 불리해집니다.")
    ...
    def a10():
        ...
        if n_ok < 20:
            return False, (
                f"★축 A 결측 종목 20개 중 {20 - n_ok}개가 탈락했습니다. §7.2 는 'ΔTONE_resid=0(중립)으로 두고 DART_SCORE 만으로 평가, 탈락시키지 말 것' 을 규정합니다.")
    ...
    def a11():
        ...
        if n_ok < 30:
            return False, (
                f"★D1 결측 종목 30개 중 {30 - n_ok}개가 탈락했습니다. §6.5 는 'D1 가중치를 D2·D3 에 비례 재배분, 종목을 탈락시키지 말 것' 을 규정합니다.")
    ...
    def a12():
        ...
        if not (c_before > 0.5 and c_after < 0.15):
            return False, (
                f"★직교화가 통제변수와의 상관을 제거하지 못했습니다 ({c_before:.3f} → {c_after:.3f}). 잔차화가 형식적으로만 수행되고 있을 가능성이 큽니다.")
    ...
    def a15():
        ...
        if prev_quarter_of("2019-09-01") != "2019Q2":
            return False, f"★리밸일 직전 분기 계산 오류: 2019-09-01 → {prev_quarter_of('2019-09-01')} (기대 2019Q2)."
    ...
    def a18():
        ...
        if re.search(pat, src, re.I):
            return False, (
                "★LLM API 호출 흔적이 발견되었습니다. §5.1/§6.3 은 이를 명시적으로 금지합니다 — 토큰 비용이 아니라 사전학습 코퍼스로 인한 룩어헤드 오염 때문입니다.")
    ...
    def a19():
        ...
        if kd <= as_ts("2020-03-20"):
            return False, (
                f"★T+1 위반: 접수일 2020-03-20 인 재무제표의 knowledge_date 가 {str(kd)[:10]} 입니다. 접수 당일부터 쓸 수 있으면 미래누수입니다(§4).")
    ...
    def a20():
        ...
        for var, why in (("_d1_on", "D1"), ("_axis_a_on", "축 A")):
            if var not in src:
                return False, f"★게이트가 {why} 를 실제로 끄지 못합니다. skip_if 는 표시만 하므로 with 본문 안에서 `if {var}:` 로 분기해야 합니다."
    ...
    def a21():
        ...
        if Q1.loc[:29, "FINAL_SCORE"].notna().any():
            return False, (
                "★축 A 단독 팔에서 ΔTONE 결측 종목에 점수가 부여됐습니다. 그 팔에는 DART_SCORE 가 없으므로 '중립 0' 은 곧 전 종목 동점이고, 리포트가 없는 종목이 코드 순서로 편입됩니다 — A1 이 축 A 의 순기여가 아니라 동점 처리 규칙을 측정하게 됩니다.")
        ...
        if Q2.loc[:14, "FINAL_SCORE"].notna().any():
            return False, "★축 A·축 B 가 모두 결측인 종목에 점수가 부여됐습니다. 아무 근거도 없는 종목이 중간 순위를 차지하고, 표본이 얇은 분기에는 실제로 편입됩니다."
    ...
    def a22():
        ...
        if got[0] < 1 or got[1] < 1:
            return False, (
                "★감사의견 '의견거절'과 CB 발행이 걸린 종목이 다음 분기보고서 접수만으로 제외 해제됐습니다. 소스별 행이 서로의 플래그를 NaN 으로 덮고, as-of 결합이 최근 1행만 붙이기 때문입니다 — §6.4 하드 제외가 사실상 작동하지 않습니다.")
        if got[2] > 0:
    ...
    def a23():
        ...
        if w.empty or not np.isfinite(float(w["fwd_ret_1q"].iloc[0])):
            return False, (
                "★다음 분기에 U-1000 을 이탈하는 종목의 전방수익률이 NaN 입니다. run_backtest 의 elig 필터가 그 종목을 **오늘의 편입 후보에서** 지우므로, 오늘의 편입 자격이 내일의 유니버스 잔류 여부로 결정됩니다(방향성 있는 편향).")
    ...
    def a24():
        ...
        if abs(g.get("MRG", -9) + 1.0) < 1e-6:
            return False, (
                "★합병·완전자회사화 같은 정상 폐지에 -100% 가 붙었습니다. 실제로는 합병비율·공개매수가로 원금 수준이 회수됩니다 — 폐지목록의 34%가 정상 사유이므로 상시 가짜 손실이 됩니다.")
        if abs(g.get("MRG", 0) - (98.0 / 100.0 - 1.0)) > 1e-6:
            return False, f"정상 폐지 청산가가 폐지일 직전 종가가 아닙니다: {g.get('MRG')}"
        if not (g.get("HLT", 0) < -0.5):
            return False, (
                "★거래정지 후 폐지 종목의 손실이 계상되지 않았습니다. 거래정지 → 시총 격자 소실 → 폐지 순서라 패널에 행이 없어지고, 한국의 감사의견거절·자본잠식 폐지는 대부분 이 경로입니다 — 손실만 선택적으로 사라집니다.")
    ...
    def a25():
        ...
        if "hash(w)" in inspect.getsource(_ToneNaiveBayes):
            return False, (
                "★폴백 TONE 분류기가 파이썬 내장 hash() 를 씁니다. CPython 의 문자열 해시는 PYTHONHASHSEED 로 프로세스마다 랜덤화되므로 같은 SEED·같은 캐시로 두 번 돌리면 보유 종목이 달라집니다.")
    ...
    def a26():
        ...
        if d > 1e-6:
            return False, (
                f"★2025년 데이터를 바꿨더니 2016년 D2_SCORE 가 달라졌습니다 (최대 {d:.4f}z). 윈저라이즈 경계·강제최하위 값이 전 기간 백분위로 잡혀 있어, 과거 관측치의 클리핑 한계가 미래로 정해집니다.")
    ...
    def a27():
        ...
        if ratio > 1.6:
            return False, f"★축 A 보유 종목이 상위 {K} 를 {ratio:.2f}배 과대점유합니다 (커버리지 {cov:.0%} · 상위 점유 {share:.0%})."
    ...
    def a28():
        ...
        if cost_q > expect * 1.5:
            return False, (
                f"★분기 비용 {cost_q:.4f} 가 기대치 {expect:.4f} 의 1.5배를 넘습니다. 전량 매도 종목이 advmap 에 없어 슬리피지가 일괄 2% 폴백으로 매겨지고 있습니다 — 회전율이 다른 어블레이션 팔이 부당한 벌점을 받습니다.")
    ...
    def a29():
        ...
        if abs(st["Sortino"] - want) > 0.05:
            return False, (
                f"★Sortino {st['Sortino']:.2f} 가 정의값 {want:.2f} 와 다릅니다. 하방편차를 '음수 수익률들의 자기 평균 대비 표본표준편차'로 계산하면 손실의 크기가 아니라 균일함을 보상하게 되어 손실이 뭉친 팔이 세 자리 Sortino 로 최우수처럼 보")
    ...
    def a30():
        ...
        if pd.notna(H.loc[1, "DELTA_NONFIN"]):
            return False, (
                "★전 태그가 '모름'인 법인의 ΔNONFIN 이 0.0 으로 계산됐습니다. 문서 파싱에 실패한 법인이 '사실이 하나도 없는 법인'과 같은 척도로 z-scoring 되어, D3_SCORE 가 사실 건수가 아니라 데이터 커버리지의 함수가 됩니다(최종 점수의 10%).")
    ...
    def a31():
        ...
        if np.isfinite(thin) and thin < 0.80:
            return False, f"★관측 17개 · 파라미터 {p + 1}개에서 잔차가 원신호를 {1 - thin:.0%} 만큼 먹었습니다(corr={thin:.2f})."
        n_ok = corrs[200][1]
        if n_ok == 0:
            return False, "관측 200개에서도 잔차가 생성되지 않습니다(가드가 과도)"
        return True, (
            f"자유도 부족(n=17) 시 잔차 미생성 · n=60 corr {corrs[60][0]:.2f} · n=200 corr {corrs[200][0]:.2f} (하한 {OLS_MIN_OBS_PER_PARAM}×파라미터)")
    ...
    def a32():
        ...
        if got == 0:
            return False, (
                "★KRX 스냅샷이 없는 실행에서 시총이 0건 확보됐습니다. DART 주식총수와 월말 종가가 둘 다 정상인데도 code 가 category dtype 이라 폴백 경로 ②③ 의 merge_asof 가 MergeError 로 죽습니다 → U-1000 0행 → build_ar")
    ...
    def a33():
        ...
        if np.isfinite(r1) and r1 > 1.0:
            return False, (
                f"★1분기 전방수익률이 {r1:+.1%} 입니다. 거래정지가 풀려 재개된 종목인데도 '정지 후 폐지'로 오인해 **3년 9개월 뒤 폐지일 직전 종가**를 1분기 수익률 자리에 넣었습니다.")
    ...
    def a34():
        ...
        try:
            ...
            return False, (
                f"★{type(e).__name__} — 육안 검증용 표 함수가 데이터 파이프라인을 끊습니다. 이 함수는 L1.DOC 안에서 ctx['doc_pairs'] 설정 **앞에** 호출되므로, 여기서 죽으면 GATE_4/5 가 판정불가가 되어 원문을 전부 받아 놓고도 D1 축이")
    ...
    def a35():
        ...
        if d > 1e-6:
            return False, f"★11월 코호트(3분기보고서)만 바꿨는데 3월 코호트(사업보고서)의 D1_SCORE 가 최대 {d:.3f}z 움직였습니다."
    ...
    def a36():
        ...
        if pd.notna(base.get("00000000")) and pd.isna(withf.get("00000000")):
            return False, (
                "★문서 접수 9개월 뒤의 합병 공시로 과거 문서의 D1 이 지워졌습니다. '앞으로 12개월 안에 합병을 공시할 기업' 이라는 미래 정보로 그 종목의 스코어 구성(§6.5 재배분)이 바뀝니다 — 합병 대상 종목은 전방수익률이 체계적으로 다르므로 방향성 있는 편향입니다.")
    ...
    def a37():
        ...
        if not _d3_tokens_hit(toks, "NF_OVERSEAS"):
            return False, (
                "★'해외'+'법인' 토큰이 둘 다 있는데 NF_OVERSEAS 가 발화하지 않습니다. 두 단어 패턴을 JSON 토큰 덤프 문자열에 정규식으로 걸면 두 토큰 사이에 항상 '\": 3, \"' 가 끼어 **원리상 매칭될 수 없습니다** — 해당 이벤트가 어떤 데이터에서도 영구")
        ...
        for k in sorted(_D3_WEAK_CONSTANT):
            if k in H.columns and pd.to_numeric(H[k], errors="coerce").notna().any():
                return False, (
                    f"★원문 없이 단일 토큰만으로 {k} 가 판정됐습니다. '이 단어가 보고서 어딘가에 나오는가'는 이벤트가 아니라 상수입니다(실측 발화율 67%) — 0/1 이 아니라 결측이어야 합니다.")
    ...
    def a38():
        ...
        if not vb.empty:
            return False, f"★현금성자산이 결측인데 NOA 가 {float(vb.iloc[0]):.4f} 로 계산됐습니다(현금 있을 때 {float(va.iloc[0]):.4f})."
        return True, f"현금 결측 → NOA 결측(§6.5 재배분) · 현금 있으면 {float(va.iloc[0]):.4f}"

    _ac("A38", "NOA 현금 결측 처리 (§0.5)", a38)

    # ── A39  공용 캐시가 수집 실패로 훼손되지 않는가 (절대 원칙) ──────────────────────────
    def a39():
        import tempfile as _tf
        with _tf.TemporaryDirectory() as td:
            V = Vault(td, mode="local")
            good = pd.DataFrame({"code": [f"{i:06d}" for i in range(500)], "v": range(500)})
            p1 = V.put_table("t_guard", good, scope="shared", domain="test")
            if not p1:
                return None, "테이블 저장 자체가 실패해 건너뜁니다"
            # 수집 실패 시나리오: 빈 프레임이 돌아온다
            V.put_table("t_guard", good.iloc[0:0], scope="shared", domain="test")
            back = V.get_table("t_guard", scope="shared")
            if back is None or len(back) != 500:
                return False, (
                    "★수집 실패(빈 프레임)가 공용 캐시를 덮어썼습니다. 백업은 남아도 살아 있는 테이블이 비어, 다음 실행의 get_table 이 그 빈 값을 읽어 조용히 열화된 상태로 돕니다 — 사용자의 절대 원칙('구글드라이브 인덱스를 훼손하지 마라') 위반입니다.")
            # 정상 증가는 통과해야 한다
            more = pd.DataFrame({"code": [f"{i:06d}" for i in range(600)], "v": range(600)})
            V.put_table("t_guard", more, scope="shared", domain="test")
            back2 = V.get_table("t_guard", scope="shared")
            if back2 is None or len(back2) != 600:
                return False, "정상 증가분이 저장되지 않았습니다(가드가 과도)"
            # 의도한 축소는 allow_shrink 로 통과
            V.put_table("t_guard", good.iloc[:10], scope="shared", domain="test",
                        allow_shrink=True)
            back3 = V.get_table("t_guard", scope="shared")
            if back3 is None or len(back3) != 10:
                return False, "allow_shrink=True 인데도 축소가 반영되지 않았습니다"
            # 삭제 API 가 없어야 한다 (인덱스 훼손 경로 원천 차단)
            for bad in ("delete", "remove", "drop_table", "purge"):
                if hasattr(V, bad):
                    return False, f"★Vault 에 삭제 API '{bad}' 가 있습니다 — 인덱스 훼손 경로입니다"
        return True, ("빈 프레임 덮어쓰기 차단(500행 보존) · 정상 증가 반영(600행) · "
                      "allow_shrink 명시 시에만 축소 · 삭제 API 부재 확인")

    _ac("A39", "공용 캐시 훼손 방지 (절대 원칙)", a39)
    ...
    LOG.table(rows, ["계약", "내용", "판정", "상세"], ["l", "l", "c", "l"], maxw=82,
              title="계약 자동검정 A1~A39 (협상 대상이 아님)")
